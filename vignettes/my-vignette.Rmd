...
```{r}
directories <- list.dirs("../../Immunizations", recursive = F)[2:5]
```
...
mut_plots <- plot_grid(plots[[1]] + theme(legend.position="none"), 
                       plots[[2]] + theme(legend.position="none"),
                       labels = c("A", "B"))
legend_b <- get_legend(plots[[1]] + theme(legend.position="right"))
mut_plots <- plot_grid(mut_plots, legend_b, ncol = 1, rel_heights = c(1, .15))
mut_plots
...
for (enc in c("onehot", "protr", "protr.cdr3", "kmer")) {
    AUC[[enc]] <- classify_data(features_cell, encoding = enc, unique.sequences = "cdr3s_aa")[["roc"]]
}
...
for (enc in c("onehot", "protr", "protr.cdr3", "kmer")) {
    reg[[enc]] <- predict_affinity(features_cell, encoding = enc, unique.sequences = "cdr3s_aa")[["reg"]]
}
